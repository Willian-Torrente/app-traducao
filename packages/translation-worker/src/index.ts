import { sequelize, Translation } from "@app/common";
import amqplib from "amqplib";

const QUEUE_NAME = "translation_requests";

async function processMessage(msg: amqplib.ConsumeMessage, channel: amqplib.Channel) {
  ...
  try {
    ...
    translation = await Translation.findByPk(id);
    ...
    console.error(
      `[Worker] Error processing message for translation ID ${id}:`,
      error
    );
...
async function startWorker() {
  try {
    ...
    console.log(
      "✅ [Worker] Connection to the database has been established successfully."
    );
    ...
    console.log(
      "✅ [Worker] Connection to RabbitMQ has been established successfully."
    );
    ...
    console.log(
      `[Worker] Waiting for messages in queue: ${QUEUE_NAME}. To exit press CTRL+C`
    );
